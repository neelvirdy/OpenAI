...
public struct ChatQuery: Equatable, Codable, Streamable {
    ...
    public enum ChatCompletionMessageParam: Codable, Equatable {
        ...
        public var refusal: String? { get {
            switch self {
            case .assistant(let assistantMessage):
                return assistantMessage.refusal
            default:
                return nil
            }
        }}
        ...
        public struct AssistantMessageParam: Codable, Equatable {
            ...
            /// The refusal message generated by the model, if it refuses the request.
        ...
        public struct FunctionDefinition: Codable, Equatable {
            ...
            public class SchemaDefinition: Codable, Equatable {
                ...
                public enum JSONTypeOrArray: Codable, Equatable {
                    case single(JSONType)
                    case multiple([JSONType])

                    // Initializer to allow easy creation from a single JSONType
                    public init(_ type: JSONType) {
                        self = .single(type)
                    }

                    public init(_ types: [JSONType]) {
                        self = .multiple(types)
                    }

                    public init(from decoder: Decoder) throws {
                        let container = try decoder.singleValueContainer()
                        if let singleType = try? container.decode(JSONType.self) {
                            self = .single(singleType)
                            return
                        }
                        if let multipleTypes = try? container.decode([JSONType].self) {
                            self = .multiple(multipleTypes)
                            return
                        }
                        throw DecodingError.typeMismatch(
                            JSONTypeOrArray.self,
                            DecodingError.Context(
                                codingPath: decoder.codingPath,
                                debugDescription: "Expected JSONType or [JSONType]"
                            )
                        )
                    }
